...
window.addEventListener('load', () => {
	...
	if (e.classList.remove("no-js"), e.classList.add("js"), document.body.classList.contains("has-animations")) {
		...
		t.reveal(".feature", {
			...
			easing: "cubic-bezier(0.5, -0.01, 0, 1.005)",
			viewFactor: .2
		}), t.reveal(".scroll-text, .project > p", {
			duration: 600,
			scale: ".95",
			easing: "cubic-bezier(0.5, -0.01, 0, 1.005)",
			viewFactor: .15
...
function changeLang() {
	...
	if (lang_selected.classList.contains("english")) {
		lang_selected.innerHTML = '🇬🇧 English<p style="display:inline; font-size:10px; padding-left:10px;">&#9660</p><div id="lang_menu" onclick="changeLang()">🇷🇺 Русский</div>';
		document.getElementById("resume-content").src = "media/Alina - Resume.pdf";
		lang_box.style.marginRight = '30px';
	}
	else {
		lang_selected.innerHTML = '🇷🇺 Русский<p style="display:inline; font-size:10px; padding-left:10px;">&#9660</p><div id="lang_menu" onclick="changeLang()">🇬🇧 English</div>';
		document.getElementById("resume-content").src = "media/Alina - Resume (RUS, ENG).pdf";
